...
const Dashboard = () => {
  ...
  // Debugging: Check if auction data is loading correctly
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 md:grid-cols-3 lg:grid-cols-4 max-w-[1400px] mx-auto">
          {auctionData &&
            currentItems.map((item) =>
              item._id ? ( // Ensure _id exists before rendering
                <div key={item._id}>
                  <SingleAuction
                    name={item?.name}
                ...
                <div key={Math.random()}>
                  {/* Fallback in case _id is missing */}
                  <p className="text-red-500">Invalid Auction Item</p>
                </div>
              )
            )}
        </div>
